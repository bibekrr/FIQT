...
<https://github.com/fdbtrs/CR-FIQA/blob/8ada2ca36b6020d5aff97cf959330e52ea241843/backbones/iresnet.py>
...
class IResNet(nn.Module):
  ...
  def __init__(self,
    ...
    self.layer2 = self._make_layer(
        block, 128, layers[1], stride=2, dilate=replace_stride_with_dilation[0], use_se=self.use_se)
    self.layer3 = self._make_layer(
        block, 256, layers[2], stride=2, dilate=replace_stride_with_dilation[1], use_se=self.use_se)
    self.layer4 = self._make_layer(
        block, 512, layers[3], stride=2, dilate=replace_stride_with_dilation[2], use_se=self.use_se)
    self.bn2 = nn.BatchNorm2d(
        512 * block.expansion,
        eps=1e-05,
    )
  ...
  def _make_layer(self, block, planes, blocks, stride=1, dilate=False, use_se=False):
    ...
    layers.append(
        block(
            self.inplanes, planes, stride, downsample, self.groups, self.base_width, previous_dilation, use_se=use_se))
...
def iresnet50_identity(pretrained=False, progress=True, **kwargs):
  return _iresne_identity('iresnet50', IBasicBlock, [3, 4, 14, 3], pretrained, progress, **kwargs)
