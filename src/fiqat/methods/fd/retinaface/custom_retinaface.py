...
https://github.com/deepinsight/insightface/blob/f89ecaaa547f12127165fc5b5aefca6d979b228a/detection/RetinaFace/retinaface.py
that supports batch processing.
"""
...
class RetinaFace:
    def __init__(self,
        ...
        device_config_kind, device_config_index = device_config
        if device_config_kind == "gpu":
          self.ctx = mx.gpu(device_config_index)
          self.nms = gpu_nms_wrapper(self.nms_threshold, device_config_index)
        elif device_config_kind == "cpu":
          self.ctx = mx.cpu(device_config_index)
          self.nms = cpu_nms_wrapper(self.nms_threshold)
          assert device_config_index == 0  # NOTE This assert might not be necessary/useful and could be removed.
        else:
          raise Exception("Invalid device_config", device_config)
        # if self.ctx_id >= 0:
    ...
    def _detect_sub(self, j, adjusted_images, net_out_all, scales, flips, threshold, timea):
        proposals_list = []
        scores_list = []
        landmarks_list = []
        strides_list = []
        im = adjusted_images[j]
        im_info = [im.shape[0], im.shape[1]]
        #post_nms_topN = self._rpn_post_nms_top_n
        #min_size_dict = self._rpn_min_size_fpn
        for im_scale in scales:
            for flip in flips:
                net_out = [net_out_sub[j].reshape((1, *net_out_sub.shape[1:])) for net_out_sub in net_out_all]
                        ...
                        for diff_idx in __idx:
                            ...
                            if body.shape[1] // A == 2:  #cls branch
                                if cls_cascade or bbox_cascade:
                                    ...
                                    scores = cascade_scores  #TODO? (original version comment)
                                    cascade_sym_num += 1
                                    cls_cascade = True
    ...
    def detect(self, img, threshold=0.5, scales=[1.0], do_flip=False):
        ...
        for img in imgs:
            for im_scale in scales:
                for flip in flips:
                    ...
                    adjusted_images.append(im)
        im_tensor = np.zeros((len(adjusted_images), 3, im.shape[0], im.shape[1]))
        for j, im in enumerate(adjusted_images):
            for i in range(3):
                im_tensor[j, i, :, :] = (
        ...
        net_out_all = self.model.get_outputs()
        
        return [self._detect_sub(j, adjusted_images, net_out_all, scales, flips, threshold, timea) for j in range(len(adjusted_images))]
